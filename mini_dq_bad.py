...
# ---------------------------------------------------------------------------
# Inlined kernels (stripped-down from fla/ops/attn/parallel.py)
# ---------------------------------------------------------------------------
...
def bwd_kernel_dq(
    ...
    # first loop: key blocks strictly before the query block (no causal mask needed)
    ...
    # second loop: key blocks overlapping the query block (causal mask applied)
...
lse_f = (torch.logsumexp(s, dim=-1) * RCP_LN2).unsqueeze(-1)
...
dq_triton = torch.empty(B, T, HQ, D, dtype=k.dtype, device=device)
bwd_kernel_dq[(triton.cdiv(D, BV), triton.cdiv(T, BT), B * HQ)](
    q=q, k=k, v=v, lse=lse_f, delta=delta, do=do, dq=dq_triton,
    scale=scale, T=T, W=None,
...
ds = p * (dp - dt_f.unsqueeze(-1))
ds = ds.masked_fill(~mask, 0.)
dq_torch = (scale * (ds @ k_f)).unsqueeze(2)

print("o  diff:", (o_f - (torch.softmax(s, dim=-1) @ v_f).unsqueeze(2)).abs().max().item())
print("dq diff:", (dq_triton.float() - dq_torch).abs().max().item())
